...
function GLTFMeshGeometry({ glb, geometryKey, onWidth }: { glb: string; geometryKey: string; onWidth?: (w: number) => void }) {
  const { nodes } = useGLTF(glb);
  const mesh = nodes[geometryKey] as THREE.Mesh;
  useEffect(() => {
    if (mesh?.geometry) {
      mesh.geometry.computeBoundingBox();
      const w = mesh.geometry.boundingBox!.max.x - mesh.geometry.boundingBox!.min.x || 1;
  ...
  return <cylinderGeometry args={[1, 1, 0.5, 64]} />;
}
  ...
  return (
    <>
      ...
      >
        ...
        >
          <GLTFMeshGeometry glb={glb} geometryKey={geometryKey} onWidth={(w) => (geoWidthRef.current = w)} />
        </Suspense>
        <MeshTransmissionMaterial
...
function Lens({ modeProps, ...p }: { modeProps?: ModeProps } & MeshProps) {
  return <ModeWrapper glb="/assets/3d/lens.glb" geometryKey="Cylinder" fallbackType="cylinder" followPointer modeProps={modeProps} {...p} />;
}

function Cube({ modeProps, ...p }: { modeProps?: ModeProps } & MeshProps) {
  return <ModeWrapper glb="/assets/3d/cube.glb" geometryKey="Cube" fallbackType="box" followPointer modeProps={modeProps} {...p} />;
}
...
function Images() {
  ...
  return (
    <group ref={group}>
      <Image position={[-2, 0, 0]} scale={[3, height / 1.1]} url="/images/hero/vaishnav-s-hero.png" />
      <Image position={[2, 0, 3]} scale={3} url="/images/hero/vaishnav-s-about.jpeg" />
      <Image position={[-2.05, -height, 6]} scale={[1, 3]} url="/images/hero/vaishnav-s-hero.png" />
      <Image position={[-0.6, -height, 9]} scale={[1, 2]} url="/images/hero/vaishnav-s-about.jpeg" />
      <Image position={[0.75, -height, 10.5]} scale={1.5} url="/images/hero/vaishnav-s-hero.png" />
    </group>
  );
}
